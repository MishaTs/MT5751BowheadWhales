...
#bowhead.hn.null.bf <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
#                         convert_units = conversion.factor,
#                         formula = ~as.factor(bf))
bowhead.hn.null.size.bf1 <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
                               convert_units = conversion.factor,
                               cutpoints = histBreaks2,
                               formula = ~size + as.factor(bf))
# fitting beafort sea state as a factor performs better, so drop this
#bowhead.hn.null.size.bf2 <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
#                               convert_units = conversion.factor,
#                               cutpoints = histBreaks2,
#                              formula = ~size + bf)
bowhead.hn.null.size.reg <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
                               convert_units = conversion.factor,
                               cutpoints = histBreaks2,
                               formula = ~size + as.factor(Region.Label))
bowhead.hn.null.full <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
                           convert_units = conversion.factor,
                           cutpoints = histBreaks2,
                           formula = ~size + as.factor(Region.Label) + as.factor(bf))
#tried some interactions and it didn't work
#bowhead.hn.null.size.bf.int <- ds(data = bowhead_LT, key = "hn", adjustment = NULL,
#                                  convert_units = conversion.factor,
#                                  formula = ~size * as.factor(bf))
...
bowhead.hr.null.size <- ds(data = bowhead_LT, key = "hr", adjustment = NULL,
                           ...
                           cutpoints = histBreaks2,
                           formula = ~size)
bowhead.hr.null.size.bf1 <- ds(data = bowhead_LT, key = "hr", adjustment = NULL,
                           convert_units = conversion.factor,
                           cutpoints = histBreaks2,
                           formula = ~size + as.factor(bf))
# fitting beafort sea state as a factor performs better, so drop this
#bowhead.hr.null.size.bf2 <- ds(data = bowhead_LT, key = "hr", adjustment = NULL,
#                               convert_units = conversion.factor,
#                               cutpoints = histBreaks2,
#                               formula = ~size + bf)
bowhead.hr.null.size.reg <- ds(data = bowhead_LT, key = "hr", adjustment = NULL,
                               convert_units = conversion.factor,
                               cutpoints = histBreaks2,
                               formula = ~size + as.factor(Region.Label))
...
bowhead.hr.null.full <- ds(data = bowhead_LT, key = "hr", adjustment = NULL,
                           convert_units = conversion.factor,
                           cutpoints = histBreaks2,
                           formula = ~size + as.factor(Region.Label) + as.factor(bf))
...
summarize_ds_models(bowhead.hn.null, 
                    ...
                    bowhead.hr.herm.size, output = "plain")

# many models fail to converge, so we're left with these options
summarize_ds_models(bowhead.hn.null, 
                    bowhead.hr.null, 
                    bowhead.hn.null.size, 
                    bowhead.hr.null.size, 
                    bowhead.hr.null.size.bf1, 
                    bowhead.hr.null.size.reg, 
                    bowhead.hr.null.full, output = "plain")

# the base half-normal and size-only are the preferred models using AIC
par(mfrow = c(2,2))
plot(bowhead.hn.null, which=2, pl.col = adjustcolor("steelblue",0.5),border=NULL, lwd = 2, 
...
# both models have solid GOF but insufficient data points to generate a plot
...
est.boot <- bootdht(model=bowhead.hn.null.size, flatfile=bowhead_LT,
...
#
#run the bootstrap
#est.bootDensitySize <- bootdht(model=bowhead.hn.null.size, flatfile=bowhead_LT,
